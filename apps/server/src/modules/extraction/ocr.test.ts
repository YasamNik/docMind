...
describe("createTesseractEngine", () => {
  ...
  it("evicts a failed worker so a later call for the same language can retry", async () => {
    let attempts = 0;
    const fakeWorker = { recognize: vi.fn(async () => ({ data: { text: "ok", confidence: 95 } })), terminate: vi.fn(async () => {}) };
    ...
    await expect(engine.recognize(new Uint8Array(), { languages: "eng", dataDir })).resolves.toEqual({
      text: "ok",
      confidence: 95,
    });
  ...
  it("returns the mean confidence alongside the recognized text", async () => {
    const fakeWorker = { recognize: vi.fn(async () => ({ data: { text: "hello world", confidence: 72.4 } })), terminate: vi.fn(async () => {}) };
    const createWorker = vi.fn(async () => fakeWorker as never);
    const engine = createTesseractEngine({ createWorker: createWorker as never });

    const result = await engine.recognize(new Uint8Array(), { languages: "eng", dataDir });

    expect(result).toEqual({ text: "hello world", confidence: 72.4 });
  });
  ...
  it("resolves terminate even when a worker failed to create, terminates the resolved worker, and creates a fresh worker afterward", async () => {
    const okWorker = { recognize: vi.fn(async () => ({ data: { text: "ok", confidence: 95 } })), terminate: vi.fn(async () => {}) };
    let createCalls = 0;
    ...
    await expect(ok).resolves.toEqual({ text: "ok", confidence: 95 });
